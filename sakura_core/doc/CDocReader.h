...
public:
	...
	wchar_t* GetAllData(ssize_t* pnDataLen);	/* 全行データを返す */
	const wchar_t* GetLineStr( CLogicInt nLine, CLogicInt* pnLineLen );
	const wchar_t* GetLineStrWithoutEOL( CLogicInt nLine, ssize_t* pnLineLen ); // 2003.06.22 Moca
	const wchar_t* GetFirstLinrStr( ssize_t* pnLineLen );	/* 順アクセスモード：先頭行を得る */
	const wchar_t* GetNextLinrStr( ssize_t* pnLineLen );	/* 順アクセスモード：次の行を得る */
