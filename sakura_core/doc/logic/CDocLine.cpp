...
void CDocLine::SetEol(bool bEnableExtEol)
{
	...
	const ssize_t nLenS = m_cLine.GetStringLength();
	const size_t nLength = (nLenS <= 0) ? 0u : static_cast<size_t>(nLenS);
	//改行コード設定
...
{
	const size_t u = (nLength < 0) ? 0u : static_cast<size_t>(nLength);
	m_cLine.SetString(pData, u);
