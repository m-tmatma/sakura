...
*/
static ssize_t NextWordBreak( const CStringRef& str, const ssize_t start );
...
bool CColor_KeywordSet::BeginColor(const CStringRef& cStr, ssize_t nPos)
{
	if( ! cStr.IsValid() ) {
	...
	*/
	...
	const ssize_t posNextWordHead = NextWordBreak( cStr, nPos );
	for( int i = 0; i < MAX_KEYWORDSET_PER_TYPE; ++i ) {
		...
		ssize_t posWordEnd = nPos; ///< nPos...posWordEndがキーワード。
		ssize_t posWordEndCandidate = posNextWordHead; ///< nPos...posWordEndCandidateはキーワード候補。
		do {
			const int ret = GetDllShareData().m_Common.m_sSpecialKeyword.m_CKeyWordSetMgr.SearchKeyWord2( iKwdSet, cStr.GetPtr() + nPos, static_cast<int>(posWordEndCandidate - nPos) );
			if( 0 <= ret ) {
				// 登録されたキーワードだった。
...
bool CColor_KeywordSet::EndColor([[maybe_unused]] const CStringRef& cStr, ssize_t nPos)
{
	return nPos == this->m_nCOMMENTEND;
}

static inline ssize_t NextWordBreak( const CStringRef& str, const ssize_t start )
{
