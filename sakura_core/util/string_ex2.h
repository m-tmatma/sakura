...
const char*    GetNextLine  ( const char* pData, ssize_t nDataLen, ssize_t* pnLineLen, ssize_t* pnBgn, CEol* pcEol); /* CR0LF0,CRLF,LF,CRで区切られる「行」を返す。改行コードは行長に加えない */
const wchar_t* GetNextLineW ( const wchar_t* pData, ssize_t nDataLen, ssize_t* pnLineLen, ssize_t* pnBgn, CEol* pcEol, bool bExtEol); // GetNextLineのwchar_t版
