...
- [x] ファイル I/O・クリップボード・外部コマンド等で **`DWORD` や 32bit 長前提**の箇所を洗い出し、2GB 超を扱う経路ではチャンク処理または 64bit API に切り替える。（**進捗**: `CFileLoad::FileOpen` のファイルサイズ取得を **`GetFileSizeEx`** に変更。`CClipboard::SetText` で **`GlobalAlloc` サイズの乗算・加算オーバーフロー**を検出して失敗させる。**進捗（追記）**: `COutputStream::Write`（`io/CStream.h`）と **`CBinaryInputStream::Read`（`io/CBinaryStream.cpp`）** は **`fwrite`/`fread` を `DWORD` 最大バイト幅で分割**（巨大バッファ1回呼び出しを避ける）。`CTextOutputStream` の **`fwrite` を `Write` に統一**。外部コマンドのパイプ `ReadFile` は従来どおり小チャンク。**蓄積側**（`CEditView_ExecCmd`）は静的 5KiB バッファで蓄積なし・安全確認済み。**進捗（追記 2）**: `CDlgTagJumpList.cpp` のタグファイル binary search の `fseek(fp, 0, SEEK_END)` を **`_fseeki64(fp, 0LL, SEEK_END)`** に変更（2GB 超タグファイルへの対応）。）
...
- **ストリーム（CRT `fread`/`fwrite`）**  
  [x] `COutputStream::Write` / `CBinaryInputStream::Read` で **`DWORD` 幅のチャンク**に分割（保存・バイナリ読み込みで極端に大きな1回要求に対応）。`CTextOutputStream` は `Write` 経由に統一。
...
- **ストリーム**: `COutputStream::Write` / `CBinaryInputStream::Read` で **`fwrite`/`fread` を `DWORD` 最大幅チャンクに分割**。`CTextOutputStream` の直接 `fwrite` を `Write` に寄せた。
